...
for dataset in ['10X_PBMC','mouse_bladder_cell','mouse_ES_cell','human_kidney_counts','Adam','Human_pancreatic_islets']:
    ...
    for decoder in ['Dot','GMF']:
        print('----------------use decoder:{} ----------------- '.format(decoder))
        ...
        for t in range(times):
            ...
            ###training
            adata, record = run_scgraphne(adata, cl_type='cl_type', decoder=decoder, return_all=True)
            print(adata)
        ...
        ## save results
        i = ['Dot','GMF'].index(decoder)
        ii = ['mse','gmf'][i]
        np.savez(os.path.join(dir0,"results/ablation/{}/{}10_{}_{}.npz".format(dataset,ii,dataset,method)),
