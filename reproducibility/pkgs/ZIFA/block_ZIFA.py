...
def testInputData(Y):
    ...
    if column_is_all_zero.sum() > 0:
        print(
            "Warning: Your Y matrix has %i columns which are entirely zero; filtering these out before continuing." % (
                column_is_all_zero.sum()))
...
def runEMAlgorithm(Y, K, singleSigma=False, n_blocks=None):
    ...
    print(
        'Running block zero-inflated factor analysis with N = %i, D = %i, K = %i, n_blocks = %i' % (N, D, K, n_blocks))
    ...
    while n_iter < max_iter:
        for i in range(N):
            ...
            for block in range(n_blocks):
                ...
                block_EZ, block_EZZT, block_EX, block_EXZ, block_EX2 = Estep(np.array([Y_to_use]), A_to_use, mus_to_use,
                                                                             sigmas_to_use, decay_coef)
            ...
            EZ[i], EZZT[i], EX[i], EXZ[i], EX2[i] = combineMatrices(y_indices_to_use[i], block_EZs, block_EZZTs,
                                                                    block_EXs, block_EXZs, block_EX2s)
...
def fitModel(Y, K, singleSigma=False, n_blocks=None, p0_thresh=.95):
    ...
    print(
        'Filtering out all genes which are zero in more than %2.1f%% of samples. To change this, change p0_thresh.' % (
                    p0_thresh * 100))
    ...
    while results is None:
        ...
        print(
            'Previously, maximum fraction of zeros for a gene was %2.3f; now lowering that to %2.3f and rerunning ZIFA' % (
            max_zero_frac, new_max_zero_frac))
