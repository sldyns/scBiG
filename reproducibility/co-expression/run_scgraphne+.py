...
for dataset in ['fetal_liver_atlas','Trachea']:
    ...
    if dataset == 'fetal_liver_atlas':
        ...
        Y = pd.read_csv(os.path.join(dir0, 'datasets/co-expression/{}/{}_label.csv').format(dataset, dataset),
                        header=None).values
    ...
    ###training
    adata, record = run_scgraphne(adata, cl_type='cl_type', gene_similarity = True,
